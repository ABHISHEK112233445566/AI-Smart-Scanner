// ============================================================
// AI SMART SCANNER — GOOGLE SHEET UPLOADER V10
// Dashboard: TOP 5 TRADE candidates, score-independent
// ============================================================
...
function selectDashboardRows(rows=[]){
  // IMPORTANT: dashboard is a ranked presentation of actual TRADE decisions.
  // Do NOT filter by score, confidence, R:R, is85Plus or is90Plus here.
  // Those quality gates remain in the decision engine. This function only
  // selects the best five rows that the engine has already classified TRADE.
  return (Array.isArray(rows)?rows:[])
    .filter(tradeEligible)
    .sort((a,b)=>{
      const ar=n(a.rankingScore??a.finalScore??a.aiFinalScore??a.score)??0;
      const br=n(b.rankingScore??b.finalScore??b.aiFinalScore??b.score)??0;
...
module.exports={updateGoogleSheet,postToGoogleSheet,getGoogleSheetUrl,selectDashboardRows,tradeEligible,rr,score,magnitude,direction,normalizeSignedScore,buildAccuracyRow,postAccuracyRows,buildScannerStatus,DASHBOARD_MAX_ROWS,MIN_CONFIDENCE,MIN_RR,addOIMood};