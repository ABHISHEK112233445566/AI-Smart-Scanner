...
const OPTION_COLUMNS = [
  ...
  "oiSentiment","callOI","putOI","pcr","optionEntry","optionLTP","optionStopLoss",
  "optionTarget1","optionTarget2","optionRisk","optionReward","optionRiskReward",
  "optionInstrumentKey","timestamp"
];
...
function bull(row) { const d = String(s(row, ["direction", "stockDirection", "technicalDirection", "finalDirection"], "")).toUpperCase(); return d === "BULLISH" || d === "CALL" || yes(row, ["isBullish"]); }
function trend(row, k) { return String(row?.[k] ?? "").toUpperCase() === "BULLISH"; }
function mtf(row) { const x = n(row, ["mtfAvailableCount"], NaN); if (Number.isFinite(x) && x > 0) return x; return ["dailyTrend", "fourHourTrend", "oneHourTrend", "fifteenMinTrend"].filter(k => trend(row, k)).length; }
function emaQuality(row) { const p=price(row),e20=n(row,["ema20"]),e50=n(row,["ema50"]),e200=n(row,["ema200"]); return p>0&&(!e20||p>=e20)&&(!e50||p>=e50)&&(!e200||p>=e200)&&(!e50||!e200||e50>=e200); }
...
function idea(row,horizon) {
  const p=price(row),s1=support(row,1),s2=support(row,2),s3=support(row,3),r1=resistance(row,1),r2=resistance(row,2),r3=resistance(row,3);
  const entry0=n(row,["stockEntry","entry"],p),sl0=n(row,["stockStopLoss","stopLoss"]),vol=yes(row,["volumeConfirmed","volumeConfirmation","volumeConfirm"]),br=s(row,["breakoutType","breakout"],"NONE");
  let entry,sl,t1,t2,buyZone,period,quality,reason;
  if(horizon==="SHORT TERM"){entry=entry0>0?entry0:p;sl=sl0>0&&sl0<entry?sl0:(s1>0&&s1<entry?s1:"");t1=r1>entry?r1:(r2>entry?r2:"");t2=r2>t1?r2:(r3>t1?r3:"");buyZone=s1>0&&s1<entry?`${s1} - ${entry}`:entry;period="Days to few weeks";quality=vol||String(br).toUpperCase()!=="NONE"?"STRONG":"GOOD";reason="Bullish daily setup with short-term timeframe alignment and a valid equity buying structure.";}
  else{entry=p;sl=sl0>0&&sl0<entry?sl0:(s2>0&&s2<entry?s2:(s1>0&&s1<entry?s1:""));t1=r2>entry?r2:(r1>entry?r1:"");t2=r3>t1?r3:(r2>t1?r2:"");buyZone=s1>0&&s2>0?`${Math.min(s1,s2)} - ${Math.max(s1,s2)}`:(s1>0?s1:entry);period="Months to years";quality=emaQuality(row)?"STRONG":"GOOD";reason="Bullish daily structure with long-term EMA support and a suitable accumulation setup.";}
...
function buildEquity(rows){const out=[],seen=new Set();for(const row of rows){const key=stock(row);if(!key||seen.has(key))continue;if(shortCandidate(row)){out.push(idea(row,"SHORT TERM"));seen.add(key);continue;}if(longCandidate(row)){out.push(idea(row,"LONG TERM"));seen.add(key);}}const q={STRONG:2,GOOD:1};return out.sort((a,b)=>(q[b.setupQuality]-q[a.setupQuality])||((b.rsi||0)-(a.rsi||0))).slice(0,EQUITY_MAX_ROWS).map((r,i)=>({rank:i+1,...r}));}

function optionType(row){const x=String(s(row,["optionType","optionsType","option_type","type","instrument_type","side","direction"],"")).toUpperCase();if(x==="CALL"||x==="CE"||x.includes("CALL"))return"CALL";if(x==="PUT"||x==="PE"||x.includes("PUT"))return"PUT";return"";}
...
function optionValue(row,col){
  const aliases={
    stock:["symbol","tradingSymbol"],symbol:["stock","tradingSymbol"],price:["ltp","lastPrice","currentPrice","close"],
    entry:["stockEntry"],stopLoss:["stockStopLoss"],target1:["stockTarget1"],target2:["stockTarget2"],riskReward:["rr","stockRiskReward"],
    optionType:["optionsType","option_type","type","side"],optionSymbol:["tradingsymbol","tradingSymbol","option_symbol"],optionExpiry:["expiry","expiryDate"],
    recommendedStrike:["strike","strikePrice"],optionStrike:["strike","strikePrice","recommendedStrike"],optionsDecision:["decision","optionDecision","tradeDecision"],
...
async function updateStrategySheets(scannerData,optionDecisions){
  ...
  const equity=buildEquity(scanner),equityRows=equity.map(r=>EQUITY_COLUMNS.map(c=>clean(r[c])));
  const unique=optionRows(options),calls=unique.filter(r=>optionType(r)==="CALL"),puts=unique.filter(r=>optionType(r)==="PUT");
