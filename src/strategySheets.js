...
function stock(row) {
  return String(s(row, ["stock", "symbol", "tradingSymbol", "name"], "")).trim().toUpperCase();
}
...
function idea(row, horizon) {
  ...
  if (horizon === "SHORT TERM") {
    ...
    entry = p;
    sl = sl0 > 0 && sl0 < entry ? sl0 : (s2 > 0 && s2 < entry ? s2 : (s1 > 0 && s1 < entry ? s1 : ""));
...
function buildEquity(rows) {
  ...
  return out.sort((a, b) => (q[b.setupQuality] - q[a.setupQuality]) || ((b.rsi || 0) - (a.rsi || 0))).slice(0, EQUITY_MAX_ROWS).map((r, i) => ({ rank: i + 1, ...r }));
}

// ============================================================
// OPTIONS: BUY-ONLY OUTPUT FLOW
// Engine decides -> contract validated -> live LTP validated
// -> market RR/confidence/gates validated -> CALL/PUT sheet.
// WATCH and REJECT are never written to option sheets.
// ============================================================
...
function decision(row) {
  return String(s(row, ["optionsDecision", "decision", "optionDecision", "tradeDecision"], "")).toUpperCase();
}
...
function failedGateCount(row) {
  ...
  if (row?.tradeGates && typeof row.tradeGates === "object") {
    return Array.isArray(row.tradeGates.failedGates) ? row.tradeGates.failedGates.length : 0;
  }
  return Array.isArray(row?.failedGates) ? row.failedGates.length : 0;
}

function allTradeGatesPassed(row) {
...
function optionBuyCandidate(row) {
  ...
  // 1) The decision engine must explicitly approve the trade.
  if (decision(row) !== "TRADE") return false;

  // 2) A real option contract and real-time option premium are mandatory.
  ...
  // 3) Underlying market structure must be valid. This is the source of
  //    entry/SL/T1/T2 and RR; option premium levels are NOT fabricated.
  if (row.marketSetupValid !== true && String(row.marketSetupValid).toLowerCase() !== "true") return false;
  if (!(riskReward(row) >= 1.5)) return false;

  // 4) Every trade gate must pass. This catches direction, MTF,
  //    confidence and scanner-quality failures consistently.
  if (!allTradeGatesPassed(row)) return false;

  // 5) Final confidence threshold.
  if (confidence(row) < 82) return false;
...
async function postSheet(sheet, headers, rows) {
  ...
  if (res?.data?.success === false) {
    throw new Error(`Google Sheets rejected ${sheet}: ${res.data.error || "unknown error"}`);
  }
  return res?.data || {};
}
...
async function updateStrategySheets(scannerData, optionDecisions) {
  ...
  // Replace all three strategy sheets every scan. Empty arrays intentionally
  // clear stale opportunities from previous scans.
