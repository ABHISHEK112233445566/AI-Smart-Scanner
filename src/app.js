...
async function main() {
    ...
    const broker = brokerMode === "UPSTOX"
        ...
        : brokerMode === "ANGELONE" || brokerMode === "ANGEL_ONE" ? angelOne : createFailoverBroker(upstox, angelOne);
    setBroker(broker);
    ...
    if (!Array.isArray(symbols) || symbols.length === 0) throw new Error("No stock symbols are enabled in the V4 universe configuration.");
    ...
    console.log("\n========== FINAL TOP 5 ==========");
    finalTop5.forEach((option, index) => console.log(`${index + 1}. ${option?.stock || "N/A"} | ${option?.optionType || "N/A"} | Strike: ${option?.recommendedStrike ?? "N/A"} | Confidence: ${option?.optionsConfidence ?? option?.confidence ?? 0} | ${option?.optionsDecision || option?.decision || "N/A"}`));

    try {
        if (typeof updateGoogleSheet === "function") {
            await updateGoogleSheet(optionDecisions, qualifiedStocks);
        }
    } catch (error) { console.error(`⚠️ Google Sheet update failed: ${error?.message || error}`); }
