...
const dividendUniverseSymbols=DIVIDEND_LONG_TERM_SYMBOLS.filter(s=>!equityScanUniverse.includes(s));
console.log(`DIVIDEND EQUITY PIPELINE: mandatory=${DIVIDEND_LONG_TERM_SYMBOLS.length} | additional scan=${dividendUniverseSymbols.length}`);
const equityScan=await scanInBatches(equityScanUniverse);
const equityScannerData=equityScan.allResults;
const dividendScan=await scanInBatches(dividendUniverseSymbols);
