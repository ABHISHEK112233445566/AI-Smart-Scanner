...
function selectDashboardCandidates(rows){
  ...
  const list=[...(Array.isArray(rows)?rows:[])].filter(Boolean).map(r=>{
...
// The preflight is deliberately before Top-20 so invalid contracts (zero strike,
// zero LTP, missing instrument key, or insufficient option liquidity) never reach
// the expensive scanner/AI stage.
...
const scanUniverse=top20.map(x=>x.symbol).filter(Boolean);
console.log(`OPTION PIPELINE: live Top-500=${top500.length} | F&O Top-100=${top100.length} | preflight-valid=${optionPreflight.length} | Top-20=${scanUniverse.length}`);
const top20Scan=await scanInBatches(scanUniverse);
const equityScannerData=top20Scan.allResults;
const optionUniverseRows=top20Scan.allResults;
