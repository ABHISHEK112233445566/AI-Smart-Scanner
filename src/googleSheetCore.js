...
function getGoogleSheetUrl(){
    const raw=process.env.GOOGLE_SHEET_WEBHOOK_URL||
...
async function postToGoogleSheet(payload){
    ...
    try{
        ...
        if(r.status<200||r.status>=300){
            ...
            throw new Error(`Google Apps Script HTTP ${r.status} for action ${payload?.action||'unknown'} / sheet ${payload?.sheet||'n/a'}: ${preview}`);
        }
        if(r.data&&typeof r.data==='object'&&r.data.success===false){
            throw new Error(`Google Apps Script rejected action ${payload?.action||'unknown'} / sheet ${payload?.sheet||'n/a'}: ${r.data.error||'unknown error'}`);
        }
        return r;
    }catch(e){
        if(e?.response){
            const body=typeof e.response.data==='string'?e.response.data:JSON.stringify(e.response.data??{});
            throw new Error(`Google Apps Script request failed: HTTP ${e.response.status||'unknown'}: ${body.replace(/\s+/g,' ').slice(0,300)}`);
        }
        throw e;
    }
}
