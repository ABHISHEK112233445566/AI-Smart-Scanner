...
function reachedLevel(direction, price, level, type) {
    ...
    if (direction === "CALL") {
        return type === "TARGET" ? price >= level : price <= level;
    }
    if (direction === "PUT") {
        return type === "TARGET" ? price <= level : price >= level;
    }
...
function evaluateAccuracy(record, candles, evaluatedAt = new Date()) {
    if (!record || !Array.isArray(candles) || candles.length === 0) {
        return record;
    }
    ...
    for (const candle of candles) {
        ...
        const closeTime = candle?.time ? new Date(candle.time) : evaluatedAt;
    ...
    if (!Number.isFinite(high) || !Number.isFinite(low)) return record;
    ...
    record.maxFavorableMove = Number(favorable.toFixed(2));
    record.maxFavorableMovePercent = Number(((favorable / entry) * 100).toFixed(2));
    record.maxAdverseMove = Number(adverse.toFixed(2));
    record.maxAdverseMovePercent = Number(((adverse / entry) * 100).toFixed(2));

    const stop = number(record.stockStopLoss);
    const t1 = number(record.stockTarget1);
    const t2 = number(record.stockTarget2);

    if (reachedLevel(direction, direction === "CALL" ? low : high, stop, "STOP")) {
        record.stopLossReached = true;
        record.stopLossReachedDate = direction === "CALL" ? record.lowestStockPriceDate : record.highestStockPriceDate;
        record.stopLossReachedTime = direction === "CALL" ? record.lowestStockPriceTime : record.highestStockPriceTime;
    }

    if (reachedLevel(direction, direction === "CALL" ? high : low, t1, "TARGET")) {
        record.target1Reached = true;
        record.target1ReachedDate = direction === "CALL" ? record.highestStockPriceDate : record.lowestStockPriceDate;
        record.target1ReachedTime = direction === "CALL" ? record.highestStockPriceTime : record.lowestStockPriceTime;
    }

    if (reachedLevel(direction, direction === "CALL" ? high : low, t2, "TARGET")) {
        record.target2Reached = true;
        record.target2ReachedDate = direction === "CALL" ? record.highestStockPriceDate : record.lowestStockPriceDate;
        record.target2ReachedTime = direction === "CALL" ? record.highestStockPriceTime : record.lowestStockPriceTime;
    }
    ...
    record.evaluationStatus = record.stopLossReached ? "SL_REACHED" :
        record.target2Reached ? "T2_REACHED" :
        record.target1Reached ? "T1_REACHED" : "IN_PROGRESS";
