...
async function scanStock(stock) {
    ...
    try {
        ...
        if (stockDirection === "NEUTRAL") {
            return { stock:stockSymbol, symbol:stockSymbol, tradingSymbol:stockSymbol, instrumentKey, price, direction:"NEUTRAL", stockDirection:"NEUTRAL", technicalDirection, score:clampSignedScore(scoreData.score), scannerScore:clampSignedScore(scoreData.score), aiScore:clampSignedScore(scoreData.score), aiFinalScore:clampSignedScore(scoreData.finalScore), signal:scoreData.signal || "NO SIGNAL", oiMood:oiMood.mood || "UNKNOWN", oiSentiment:oiMood.sentiment || "UNKNOWN", oiDataAvailable:oiMood.dataAvailable === true, oiPriceChangePercent:toNumber(oiMood.priceChangePercent), oiChangePercent:toNumber(oiMood.oiChangePercent), qualified:false, rejectionReason:"NO_DIRECTION", support1:toNumber(sr.support1), support2:toNumber(sr.support2), resistance1:toNumber(sr.resistance1), resistance2:toNumber(sr.resistance2), pipeline:{preFilter:"NO_DIRECTION",momentumScore:0,mtfChecked:false,optionsChecked:false} };
        }
...
async function scanStocks(stocks) {
    ...
    for (const stock of stocks) { const result=await scanStock(stock); if(!result)continue; allResults.push(result); result.qualified?qualified.push(result):rejected.push(result); }
    qualified.sort((a,b)=>(Math.abs(Number(b.finalScore??b.score??0))-Math.abs(Number(a.finalScore??a.score??0))) || (Number(b.riskReward??0)-Number(a.riskReward??0)) || (Number(b.momentumScore??0)-Number(a.momentumScore??0)));
    ...
    Object.defineProperties(shortlist,{allResults:{value:allResults,enumerable:false},rejected:{value:rejected,enumerable:false},qualifiedAll:{value:qualified,enumerable:false}});
    return shortlist;
}
