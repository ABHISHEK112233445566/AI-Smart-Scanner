...
function validCandle(c){if(!c||!c.time)return false;const o=+c.open,h=+c.high,l=+c.low,cl=+c.close;return[o,h,l,cl].every(Number.isFinite)&&o>0&&h>=Math.max(o,cl)&&l<=Math.min(o,cl)&&h>=l;}
...
function extractLtp(q){const candidates=[q?.last_price,q?.lastPrice,q?.ltp,q?.last_traded_price,q?.lastTradedPrice];for(const v of candidates){const n=Number(v);if(Number.isFinite(n)&&n>0)return n;}return 0;}
function extractNumber(...values){for(const v of values){const n=Number(v);if(Number.isFinite(n)&&n>=0)return n;}return 0;}

async function getFullQuote(key){
    const k=await resolveInstrumentKey(key);
    if(!k)throw new Error(`Upstox instrument not found: ${key}`);
    try{
        const r=await axios.get(`${BASE_URL}/v2/market-quote/quotes`,{params:{instrument_key:k},headers:{Accept:"application/json",Authorization:`Bearer ${getAccessToken()}`},timeout:10000});
        const q=findQuote(r?.data?.data,k);
        if(!q)throw new Error(`Quote not returned for ${k}`);
        const ltp=extractLtp(q);
        if(!(ltp>0))throw new Error(`Invalid live LTP for ${k}`);
        return {instrumentKey:k,ltp,open:extractNumber(q.open),high:extractNumber(q.high),low:extractNumber(q.low),close:extractNumber(q.close,q.close_price),volume:extractNumber(q.volume,q.volume_traded,q.volumeTraded),oi:extractNumber(q.oi,q.open_interest,q.openInterest),previousOI:extractNumber(q.prev_oi,q.previous_oi,q.previousOI),timestamp:q.timestamp||q.timestamp_nanos||null,raw:q};
    }catch(e){throw new Error(`Upstox live quote failed | ${k} | ${apiError(e)}`);}
}

async function getHistoricalData(symbol,interval="ONE_DAY"){
    const n=normalizeInterval(interval);if(!INTERVALS[n])throw new Error(`Unsupported Upstox timeframe: ${interval}`);
    const key=await resolveInstrumentKey(symbol);if(!key)throw new Error(`Upstox instrument not found: ${symbol}`);
    const today=new Date(),from=new Date(today);from.setDate(from.getDate()-daysBack(n));const c=INTERVALS[n];
    const url=`${BASE_URL}/v3/historical-candle/${encodeURIComponent(key)}/${c.unit}/${c.interval}/${formatDateIST(today)}/${formatDateIST(from)}`;
    try{
        const r=await axios.get(url,{headers:{Accept:"application/json",Authorization:`Bearer ${getAccessToken()}`},timeout:20000});
        const raw=r?.data?.data?.candles;if(!Array.isArray(raw)||!raw.length)throw new Error("EMPTY_DATA");
        let candles=raw.map(a=>Array.isArray(a)&&a.length>=5?{time:a[0],open:+a[1],high:+a[2],low:+a[3],close:+a[4],volume:+(a[5]||0)}:null).filter(validCandle).sort((a,b)=>new Date(a.time)-new Date(b.time));
        if(!candles.length)throw new Error("No valid historical candles after validation");
        // Upstox daily history can contain only the last completed daily candle during market hours.
        // Overlay today's live OHLC/LTP so the scanner never treats yesterday's close as today's price.
        if(n==="ONE_DAY"){
            try{
                const live=await getFullQuote(key);
                const todayKey=formatDateIST(new Date());
                const idx=candles.findIndex(x=>formatDateIST(new Date(x.time))===todayKey);
                const liveCandle={time:live.timestamp||new Date().toISOString(),open:live.open>0?live.open:live.ltp,high:live.high>0?live.high:live.ltp,low:live.low>0?live.low:live.ltp,close:live.ltp,volume:live.volume};
                if(idx>=0)candles[idx]=liveCandle;else candles.push(liveCandle);
                candles.sort((a,b)=>new Date(a.time)-new Date(b.time));
            }catch(liveError){
                console.warn(`⚠️ Today's live overlay unavailable for ${key}: ${liveError?.message||liveError}`);
            }
        }
        return candles;
    }catch(e){throw new Error(`Upstox historical data failed | ${key} | ${n} | ${apiError(e)}`);}
}

async function getQuote(symbol){
    const q=await getFullQuote(symbol);
    return{data:{[q.instrumentKey]:{last_price:q.ltp,open:q.open,high:q.high,low:q.low,close:q.close,volume:q.volume,oi:q.oi,prev_oi:q.previousOI,timestamp:q.timestamp}},instrumentKey:q.instrumentKey};
}
async function getOptionLTP(instrumentKey){const x=await getOptionQuote(instrumentKey);return x.ltp;}
async function getOptionQuote(instrumentKey){const key=await resolveInstrumentKey(instrumentKey);if(!key)throw new Error(`Invalid option instrument key: ${instrumentKey}`);const q=await getFullQuote(key);if(!(q.ltp>0))throw new Error(`Invalid option LTP for ${key}`);return{instrumentKey:key,ltp:q.ltp,volume:q.volume,oi:q.oi,previousOI:q.previousOI,open:q.open,high:q.high,low:q.low,close:q.close,timestamp:q.timestamp,raw:q.raw};}
