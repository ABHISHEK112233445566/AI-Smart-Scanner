...
// Option objects use option-specific price/OI fields.
// ============================================================
...
function calculateOIMood({ currentPrice, previousPrice, currentOI, previousOI } = {}) {
    const price = toNumber(currentPrice), prevPrice = toNumber(previousPrice);
    const oi = toNumber(currentOI), prevOI = toNumber(previousOI);
    if (!Number.isFinite(price) || !Number.isFinite(prevPrice) || !Number.isFinite(oi) || !Number.isFinite(prevOI) || prevPrice <= 0 || prevOI <= 0) {
        return { mood: "UNKNOWN", sentiment: "UNKNOWN", priceChange: 0, priceChangePercent: 0, oiChange: 0, oiChangePercent: 0, priceDirection: "UNKNOWN", oiDirection: "UNKNOWN", dataAvailable: false };
    }
    const priceChange = price - prevPrice, priceChangePercent = priceChange / prevPrice * 100;
    const oiChange = oi - prevOI, oiChangePercent = oiChange / prevOI * 100;
    ...
    const sentiment = mood === "LONG BUILDUP" || mood === "SHORT COVERING" ? "BULLISH" : mood === "SHORT BUILDUP" || mood === "LONG UNWINDING" ? "BEARISH" : "NEUTRAL";
    return { mood, sentiment, priceChange, priceChangePercent, oiChange, oiChangePercent, priceDirection, oiDirection, dataAvailable: true };
}

function calculateOIMoodForStock(stock = {}) {
    if (!stock || typeof stock !== "object") return calculateOIMood();
    const hasOptionIdentity = Boolean(stock.optionInstrumentKey || stock.optionInstrument || stock.optionSymbol || stock.optionType || stock.recommendedStrike || stock.atmStrike);
