...
async function upstoxFullQuotes(instrumentKeys){const token=process.env.UPSTOX_ACCESS_TOKEN;if(!token)throw new Error("UPSTOX_ACCESS_TOKEN is missing");const keys=[...new Set((instrumentKeys||[]).filter(Boolean))];if(!keys.length)return[];const response=await axios.get(`${UPSTOX_BASE}/v2/market-quote/quotes`,{params:{instrument_key:keys.join(",")},headers:{Accept:"application/json",Authorization:`Bearer ${token}`},timeout:15000});return normalizeQuoteMap(response?.data?.data);}
