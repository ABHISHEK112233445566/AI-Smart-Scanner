...
async function filterOptionEligibleStocks(topRows,broker,limit=TOP_OPTION_STOCKS){
 const input=(Array.isArray(topRows)?topRows:[]).filter(row=>{const direction=String(row?.direction??row?.stockDirection??row?.technicalDirection??'').toUpperCase();const validDirection=direction==='BULLISH'||direction==='BEARISH';const entry=n(row?.entry??row?.stockEntry??row?.marketEntry),stop=n(row?.stopLoss??row?.stockStopLoss),target1=n(row?.target1??row?.stockTarget1),rr=n(row?.riskReward??row?.stockRiskReward);const setupValid=entry>0&&stop>0&&target1>0&&rr>=1.5;return validDirection&&setupValid;}).sort((a,b)=>(n(b.volume)-n(a.volume))||(n(b.liveVolumeRank)-n(a.liveVolumeRank))).slice(0,OPTION_LIQUIDITY_CANDIDATE_POOL),confirmed=[],target=Math.min(TOP_OPTION_STOCKS,Math.max(1,limit)),concurrency=8;
