// ============================================================
// AI SMART SCANNER — GOOGLE APPS SCRIPT API V11
// ============================================================
...
var ACCURACY_HEADERS=["recordId","predictionTime","symbol","stockPrice","optionType","confidence","predictedEntry","target","stopLoss","currentPrice","targetSLReached","resultTime","resultPrice","accuracyPercent"];
...
function appendRows(sheetName,headers,rows){
  ...
  var output=[];for(var i=0;i<rows.length;i++)output.push(normalizeRow(rows[i],headers.length));sheet.getRange(sheet.getLastRow()+1,1,output.length,headers.length).setValues(output);SpreadsheetApp.flush();return{rowCount:output.length,columnCount:headers.length};
}

function appendAccuracyRows(sheet,incomingHeaders,rows){
  var tz="Asia/Kolkata",todayKey=Utilities.formatDate(new Date(),tz,"yyyy-MM-dd"),today=new Date(todayKey+"T00:00:00+05:30"),cutoff=new Date(today.getTime()-24*60*60*1000),cutoffKey=Utilities.formatDate(cutoff,tz,"yyyy-MM-dd");
...
function refreshAccuracySummary(sheet){
  var lastRow=sheet.getLastRow(),rows=lastRow>=ACCURACY_DATA_START_ROW?sheet.getRange(ACCURACY_DATA_START_ROW,1,lastRow-ACCURACY_DATA_START_ROW+1,ACCURACY_HEADERS.length).getValues():[],now=new Date(),tz="Asia/Kolkata",weekStart=istanbulWeekStart(now),monthStart=istanbulMonthStart(now),week=countAccuracyPeriod(rows,weekStart,now,tz),month=countAccuracyPeriod(rows,monthStart,now,tz);
...
function istanbulWeekStart(now){var tz="Asia/Kolkata",s=Utilities.formatDate(now,tz,"yyyy-MM-dd"),d=new Date(s+"T00:00:00+05:30"),day=d.getDay(),diff=day===0?6:day-1;return new Date(d.getTime()-diff*86400000);}
function istanbulMonthStart(now){var tz="Asia/Kolkata",s=Utilities.formatDate(now,tz,"yyyy-MM" );return new Date(s+"-01T00:00:00+05:30");}
...
function updateAccuracyRows(updates){
  ...
  var updated=0,notFound=0;for(var u=0;u<updates.length;u++){var update=updates[u];if(!update||!update.recordId){notFound++;continue;}var id=String(update.recordId).trim();if(rowMap[id]===undefined){notFound++;continue;}var rowIndex=rowMap[id];if(update.targetSLReached!==undefined)data[rowIndex][10]=normalizeCell(update.targetSLReached);if(update.resultTime!==undefined)data[rowIndex][11]=normalizeCell(update.resultTime);if(update.resultPrice!==undefined)data[rowIndex][12]=normalizeCell(update.resultPrice);if(update.accuracyPercent!==undefined)data[rowIndex][13]=normalizeCell(update.accuracyPercent);updated++;}
  sheet.getRange(ACCURACY_DATA_START_ROW,1,data.length,ACCURACY_HEADERS.length).setValues(data);refreshAccuracySummary(sheet);SpreadsheetApp.flush();return{updated:updated,notFound:notFound};
}
...
function formatAccuracySheet(sheet){sheet.getRange(1,1,1,5).setFontWeight("bold");sheet.getRange(5,1,1,ACCURACY_HEADERS.length).setFontWeight("bold");sheet.setFrozenRows(5);sheet.getRange(2,5,2,1).setNumberFormat("0.00\%");sheet.getRange(ACCURACY_DATA_START_ROW,14,Math.max(1,sheet.getMaxRows()-ACCURACY_DATA_START_ROW+1),1).setNumberFormat("0.00");}
...
function doGet(){return jsonResponse({success:true,service:"AI Smart Scanner Google Sheet API",version:"V11",status:"RUNNING",lockWaitMs:LOCK_WAIT_MS,accuracyHeaderCompatible:true,liveAccuracyReadUpdate:true,performanceMode:true,accuracyColumns:ACCURACY_HEADERS.length,accuracyColumnsList:ACCURACY_HEADERS,accuracyDataStartRow:ACCURACY_DATA_START_ROW,supportedActions:["replaceSheet","appendRows","getAccuracyRows","updateAccuracy","scanner_status"],supportedSheets:Object.keys(ALLOWED_SHEETS),accuracyHistoryPolicy:"yesterday_onward_all_option_ready_stocks_deduplicated",timestamp:new Date().toISOString()});}
